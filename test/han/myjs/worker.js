 function worker(){
	  var data = [];
    for(var i=0;i<334;i++){
        data[i] =  {wid:i+1,wname:"1001",wpower:"全部",wlevel:"A",wduty:"经理",wunit:"餐饮部",wtime:"2015-5-1",opid:"12",optime:"2015-5-1", };
    }//数据在下面的url中
    $.ajax({
		url: 'http://cins.swpu.edu.cn:8003/worder',  //跨域请求请加域名前缀：http://cins.swpu.edu.cn:8003
		dataType: "jsonp",	 
		...
		success:function(data){
			 var cs = new table({
            "tableId":"cs_table",    	 
            "headers":["编号","姓名","系统权限","级别" ,"职务","部门","入职时间","操作员","操作时间" ],   //必须
            "data":data,         
            "displayNum": 6,     
            "groupDataNum":9   
           });	 
		   } //当要数据的时候去掉这些东西，从所给的地址中读取数据
 ...
 * @param options
 */
 //整个表格的构建，包括表格的的行列以及单元格的内容，表格的翻页
function table(options){
    ...
    this.init = function(begin,end){
        ...
        this.content = options.data;//数组的名称，针对不同的数据设置不同的类型
